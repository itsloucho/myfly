...
import { buildTenantResourceUrl, getStorageUrl } from '@/lib/utils';
...
export default function TicketsPage() {
  ...
  const fetchTrips = async () => {
    try {
      const data = (await tripsService.list()).filter((trip) => trip.type === 'ticket');
      setAllTrips(data);
      setTrips(data);
    } catch (error) {
  ...
  return (
    <div>
      <div className="mb-6 flex items-center justify-between">
        <div>
          <h1 className={`text-2xl font-bold ${isDark ? 'text-[#F6F4FF]' : 'text-gray-900'}`}>Tickets</h1>
          <p className="text-gray-600 dark:text-gray-400">Manage your tickets</p>
        </div>
          ...
          >
            ...
            <div className="p-4">
              ...
              <div className="flex items-center gap-2">
                <a
                  href={buildTenantResourceUrl(`/tickets/${trip.slug}`, user?.tenant?.slug) || '#'}
                  target="_blank"
                ...
                <Link href={`/billets/${trip.id}/edit`}>
      ...
      {trips.length === 0 && (
        <div className="text-center py-12">
          <p className={`mb-4 ${isDark ? 'text-gray-400' : 'text-gray-500'}`}>No tickets found</p>
          <Link href="/billets/new">
            <Button>
              <Plus size={20} className="mr-2" weight="fill" />
