...
export default function HotelsPage() {
  ...
  const fetchTrips = async () => {
    try {
      const data = (await tripsService.list()).filter((trip) => trip.type === 'hotel');
      setAllTrips(data);
      setTrips(data);
    } catch (error) {
  ...
  return (
    <div>
      <div className="mb-6 flex items-center justify-between">
        <div>
          <h1 className={`text-2xl font-bold ${isDark ? 'text-[#F6F4FF]' : 'text-gray-900'}`}>Hôtels</h1>
          <p className="text-gray-600 dark:text-gray-400">Manage your hotels</p>
        </div>
          ...
          >
            ...
            <div className="p-4">
              ...
              <div className="flex items-center gap-2">
                <a
                  href={buildTenantResourceUrl(`/hotels/${trip.slug}`, user?.tenant?.slug) || '#'}
                  target="_blank"
      ...
      {trips.length === 0 && (
        <div className="text-center py-12">
          <p className={`mb-4 ${isDark ? 'text-gray-400' : 'text-gray-500'}`}>No hotels found</p>
          <Link href="/hotels/new">
            <Button>
              <Plus size={20} className="mr-2" weight="fill" />
